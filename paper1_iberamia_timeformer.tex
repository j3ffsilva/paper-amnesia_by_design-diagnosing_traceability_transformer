...
never been tested on whether they actually achieve this geometrically. Knowing 
...
simple global period signal and joint token-time conditioning are geometrically 
equivalent under reliable local markers; the difference only appears when those 
markers are removed. Finally, for words that develop two coexisting senses, mean 
prototypes fail systematically: they collapse both senses into a centroid that 
...
Against this suite we evaluate four conditioning architectures: a non-temporal baseline, a global-signal baseline, joint token-time conditioning, and a memory-augmented extension. Our research question is whether any form of conditioning improves geometric traceability over the non-temporal baseline, and what the specific form contributes. Evaluation on noisier regimes and natural corpora is left for future work.

The paper makes four contributions. (1) We establish temporal traceability as a geometric property distinct from period-label recoverability, and motivate why standard training objectives do not directly optimize it. (2) We introduce a controlled diagnostic suite with planted trajectories — stable, drifting, and bifurcating — and a four-diagnostic battery that targets neighborhood geometry rather than label recovery. (3) Using this suite, we show that any form of temporal conditioning more than doubles neighborhood displacement for drifting subjects relative to the non-temporal baseline; however, a global period signal (Additive) suffices to reproduce this effect under reliable local markers, and joint token-time conditioning (Token-Time) only pulls ahead when those markers are removed. (4) We identify mean-prototype aggregation as a failure mode for bifurcating subjects: when a word develops two coexisting senses, a mean prototype collapses both into a centroid that represents neither.
...
queries geometrically meaningful. Any form of conditioning more than doubles
neighborhood displacement for drifting subjects relative to the non-temporal
baseline, and the diagnostic suite distinguishes all three trajectory classes
without relying on probe accuracy alone. These results open a path toward
representations in which \textit{gay}@1920 and \textit{gay}@1980 occupy
different, directly inspectable neighborhoods in a shared geometric space.

Within this controlled setting, a global temporal signal suffices to reproduce
aggregate neighborhood movement under reliable local markers: Additive and
Token-Time are equivalent on drift score (TOST, \(\delta=0.05\),
\(p_\mathrm{TOST}<0.001\)). The only reproducible distinction between
conditioning strategies appears on the ambiguous split (\(0.595\)
vs.\ \(0.579\), non-overlapping CIs), where local markers carry no signal,
suggesting that joint conditioning encodes temporal information more precisely
when context is degraded. Layer-wise CKA corroborates this: Additive
vs.\ Token-Time similarity increases from 0.726 to 0.887 across encoder depth
(Standard stays near 0.22), suggesting self-attention absorbs the input-level
difference. Adjudication between conditioning strategies across noisier regimes
and natural corpora remains open.

Two failure modes point to directions for future work. First, for bifurcating
subjects, mean prototypes collapse two coexisting senses into a centroid that
represents neither. The oracle diagnostic shows the memory pathway itself is
sound: fixed vectors encoding the true \(P(N_1 \mid s, t)\) trajectory increase
the sign-flip rate by \(+0.070\) over Token-Time (\(0.694\) vs.\ \(0.624\)).
The bottleneck is aggregation, not the pathway; future work should explore
memory mechanisms that preserve distributional structure rather than collapsing
each period to a single prototype. Second, no architecture achieves full
traceability across all diagnostics, pointing to a mismatch between current
training objectives, which reward local prediction accuracy, and the geometric
consistency across periods that traceability requires.

These conclusions come from controlled diagnostics, not open-domain diachronic
language. The probe-level advantage of Token-Time over Additive is
statistically stable but modest (1.6 percentage points) and does not appear on
the drift score; it may not generalize beyond fixed subject positions and a
small vocabulary. The suite was co-designed with the evaluated architectures to
characterize them; the paper does not claim it as an independent validation of
any architecture as a definitive solution to temporal traceability.

Two further directions remain open. Token-Time layer-0 heads show greater
...
but replication across noise regimes and natural corpora is needed before
